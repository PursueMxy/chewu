...
//            String filePath = getFilePath(context, name).toString();
    ...
    public void playRaw(Context context, int name) {
        mMediaPlayer = MediaPlayer.create(context, name);//创建mediaplayer对象
        try {
            mMediaPlayer.reset();
            mMediaPlayer = MediaPlayer.create(context, name);//重新设置要播放的音频
